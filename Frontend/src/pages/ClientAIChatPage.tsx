...
function renderMd(text: string) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (h3) { els.push(<h4 key={i} className="mt-3 mb-1 text-sm font-bold text-foreground border-l-2 border-sky-500 pl-2">{fmt(h3[1])}</h4>); continue; }
...
export default function ClientAIChatPage() {
  ...
  const handleSend = useCallback(async () => {
    ...
    try {
      ...
      if (currentFile) {
        result = await api.ai.chatWithFile(currentFile, history, user?.email || "", "client");
      } else {
        result = await api.ai.chat(history, user?.email || "", "client");
      }
