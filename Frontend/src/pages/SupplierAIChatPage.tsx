...
export default function SupplierAIChatPage() {
  ...
  const [sidebarTab, setSidebarTab] = useState<"inv" | "history">("inv");
      ...
      <div className="hidden lg:flex w-80 flex-col gap-3">
        ...
        <div className="flex rounded-xl border border-border bg-secondary/30 p-0.5 gap-0.5">
          {(["inv", "history"] as const).map((tab) => (
            ...
            >
              {tab === "inv" ? <><Package className="h-3.5 w-3.5" /> Inventory</> : <><History className="h-3.5 w-3.5" /> History {chatSessions.length > 0 && <span className="rounded-full bg-orange-500/15 px-1.5 text-[10px] text-orange-500">{chatSessions.length}</span>}</> }
            </button>
          ))}
        </div>
          ...
          <GlassCard interactive={false} className="flex flex-col flex-1 p-0 overflow-hidden card-shadow">
            <div className="flex items-center gap-2.5 border-b border-border px-4 py-3.5">
              ...
              <button
                ...
                className="flex items-center gap-1 rounded-xl border border-orange-500/30 bg-orange-500/5 px-2.5 py-1.5 text-[11px] font-medium text-orange-500 hover:bg-orange-500/10 transition-colors"
              >
                  ...
                  return (
                    <motion.div
                      ...
                      className={`group flex items-start gap-2 rounded-xl border px-3 py-2.5 cursor-pointer transition-all ${
                        isActive ? "border-orange-500/40 bg-orange-500/8" : "border-border hover:border-orange-500/20 hover:bg-orange-500/5"
                      }`}
                      onClick={() => loadSession(session.session_id)}
                    >
                      <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-orange-500/10 mt-0.5">
