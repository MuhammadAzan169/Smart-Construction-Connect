...
function renderMd(text: string) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (h3) { els.push(<h4 key={i} className="mt-3 mb-1 text-sm font-bold text-foreground border-l-2 border-orange-500 pl-2">{fmt(h3[1])}</h4>); continue; }
...
export default function SupplierAIChatPage() {
  ...
  const handleSend = useCallback(async () => {
    ...
    try {
      ...
      if (currentFile) {
        result = await api.ai.chatWithFile(currentFile, history, user?.email || "", "supplier");
      } else {
        result = await api.ai.chat(history, user?.email || "", "supplier");
      }
      ...
      <GlassCard interactive={false} className="flex flex-1 flex-col p-0 overflow-hidden card-shadow">
        ...
        <div className="border-t border-border px-5 py-4">
            ...
            <motion.button type="button" onClick={toggleRecording} disabled={isTyping}
              className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl border transition-colors disabled:opacity-40 ${
                isRecording
                  ...
                  : "border-border bg-secondary/40 text-muted-foreground hover:text-foreground hover:border-orange-500/30"
              }`}
          ...
          <p className="mt-1.5 text-center text-[10px] text-muted-foreground/50">
            Enter to send · 📎 Attach price sheets, inventory files · 🎤 Voice notes · AI may make mistakes
          </p>
        </div>
      </GlassCard>
