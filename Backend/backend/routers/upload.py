...
from backend.utils.data_handler import add_upload_record
...
async def upload_image(
    ...
    rel_path = f"company_data/{role_dir}/{safe_name}/{filename}"
    url = storage.save_bytes(rel_path, content, content_type)
...
async def upload_document(
    ...
    rel_path = f"company_data/{role_dir}/{safe_name}/documents/{filename}"
    url = storage.save_bytes(rel_path, content, content_type)
...
async def upload_gallery_image(
    ...
    # Unique suffix avoids collisions when listing isn't available (remote storage).
    idx = uuid.uuid4().hex[:8]
    filename = f"{prefix}{idx}.{ext}"

    rel_path = f"company_data/{role_dir}/{safe_name}/gallery/{filename}"
    url = storage.save_bytes(rel_path, content, content_type)
...
async def upload_message_file(
    ...
    rel_path = f"uploads/{safe_sender}/{safe_convo}/{filename}"
    url = storage.save_bytes(rel_path, content, content_type)
    ...
    # Record in the upload index so the admin file monitor works with any backend.
    from datetime import datetime, timezone
    add_upload_record({
        "filename": file.filename or filename,
        "url": url,
        "sender": sender_email,
        "conversation_id": safe_convo,
        "category": category,
        "size": len(content),
