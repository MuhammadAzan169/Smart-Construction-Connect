...
# ── Database (Supabase / Postgres document store) ─────────────────────────────
# When DATABASE_URL is set, all JSON documents persist to Postgres instead of
# local files (required for durability on Render's ephemeral disk). Use the
# Supabase "Transaction pooler" connection string (port 6543) for a small
# always-restarting web service.
DATABASE_URL: str = os.getenv("DATABASE_URL", "").strip()

# ── Supabase Storage (binary uploads: images, documents, attachments) ─────────
# When both are set, uploads go to a Supabase Storage bucket; otherwise files
# are written to local disk and served by the StaticFiles mounts.
SUPABASE_URL: str = os.getenv("SUPABASE_URL", "").strip().rstrip("/")
SUPABASE_SERVICE_ROLE_KEY: str = os.getenv("SUPABASE_SERVICE_ROLE_KEY", "").strip()
SUPABASE_BUCKET: str = os.getenv("SUPABASE_BUCKET", "media").strip()

# ── Embeddings provider (semantic search) ─────────────────────────────────────
# On low-RAM hosts (Render free tier) local PyTorch/SBERT cannot load, so the
# semantic index calls an OpenAI-compatible embeddings API instead. If these are
# empty the index gracefully falls back to BM25 keyword search.
#   EMBEDDINGS_API_URL   e.g. https://api.openai.com/v1
#   EMBEDDINGS_MODEL     e.g. text-embedding-3-small
EMBEDDINGS_API_URL: str = os.getenv("EMBEDDINGS_API_URL", "").strip().rstrip("/")
EMBEDDINGS_API_KEY: str = os.getenv("EMBEDDINGS_API_KEY", "").strip()
EMBEDDINGS_MODEL: str = os.getenv("EMBEDDINGS_MODEL", "text-embedding-3-small").strip()
# Set ENABLE_LOCAL_SBERT=1 only on a host with >=2GB RAM to use local PyTorch.
ENABLE_LOCAL_SBERT: bool = os.getenv("ENABLE_LOCAL_SBERT", "0") == "1"
...
def setup_logging() -> None:
    ...
    # Windows consoles default to cp1252 and choke on the ↔ arrows / emojis used
    # in log messages. Force UTF-8 so logging never raises. No-op on Linux/Render.
