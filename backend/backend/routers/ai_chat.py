...
from backend.utils.data_handler import (
    add_activity_log, DB_DIR, read_json, write_json, json_exists, delete_json,
    list_upload_records, remove_upload_record,
)
        ...
        if rec.get("type") == "company":
            ...
            try:
                ...
                if company:
                    ...
                    # Gallery images — sourced from the company's own project records
                    # (already-persisted URLs, correct whether they point at local
                    # disk or Supabase Storage) rather than walking the local
                    # filesystem, which is empty in production.
                    gallery_images = []
                    for project in company.get("projects", []):
                        gallery_images.extend(project.get("image_urls") or [])
                        if len(gallery_images) >= 4:
                            break
                    entry["gallery_images"] = gallery_images[:4]
            except Exception as e:
...
async def admin_list_all_files(
    ...
    # When the document store is active, binaries live in Supabase Storage — read
    # the upload index instead of walking the (empty) local uploads directory.
    if db.is_enabled():
        records = list_upload_records()
        if file_type:
            records = [r for r in records if r.get("category") == file_type]
        records.sort(key=lambda f: f.get("uploaded_at", ""), reverse=True)
        return {"files": records[:limit], "total": len(records)}
...
async def admin_delete_file(
    ...
    # Remote / DB-backed mode: delete from storage + upload index.
    if db.is_enabled() or storage.is_remote():
        storage.delete_by_url(url)
        remove_upload_record(url)
